...
def main():
    ...
    if not os.path.exists(durations_path):
        print(f"Error: {durations_path} not found! Run generate_audio_ch24.py first.")
        return
    ...
    ffmpeg_cmd = (
        f"ffmpeg -y "
        + " ".join(inputs)
        + f' -filter_complex "{filter_complex}" -map "[ap]" -c:a pcm_s16le audio/ch24_full_voiceover.wav'
    )
    ...
    merge_cmd = (
        "ffmpeg -y -i media/videos/video_scene_ch24/720p30/RustProfessionalizationVideo.mp4 "
        "-i audio/ch24_full_voiceover.wav -map 0:v -map 1:a "
        '-filter:a "loudnorm=I=-14:TP=-1.0:LRA=11" -c:v copy -c:a aac -shortest 24.mp4'
    )
    print("Merging video and normalized audio stream...")
    subprocess.run(merge_cmd, shell=True, check=True)
    print("Success! Final video with exactly 5 minutes (300.0s) duration is saved as 24.mp4")
