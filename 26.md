...
```rust
use std::fs::OpenOptions;
use std::io::Write; // Der entscheidende Import, der den Trait in den Scope bringt
...
2. **Erster Entwurf (Action)**: Der Agent schreibt den naiven Code (aus Abschnitt 4) in die Datei.
...
- [ ] Verstanden haben, warum Traits in Rust im Scope sein müssen, um deren Methoden aufrufen, und wie Agenten diesen Fehler beheben.
