...
In den bisherigen Kapiteln dieses Kurses haben Sie die grundlegenden Bausteine von Rust kennengelernt: Variablen, Datentypen, Funktionen, Kontrollstrukturen und das Ownership-System. Sie haben wahrscheinlich oft direkt mit dem Tippen von Rust-Code begonnen, sobald eine Aufgabe gestellt wurde. Bei kleinen Programmen funktioniert das gut. Sobald Ihre Projekte jedoch größer werden, stößt dieser "Code-First"-Ansatz an seine Grenzen.

In diesem Kapitel lernen wir den VS Code Planungs-Workflow kennen. Wir besprechen ausführlich, warum die Planung vor dem Schreiben von Code für professionelle Entwickler unerlässlich ist, wie man Algorithmen visuell darstellt und wie man strukturierten Pseudocode schreibt. Am Ende dieses Kapitels werden Sie in der Lage sein, komplexe Softwareprojekte systematisch zu planen und fehlerfrei in Rust umzusetzen.
...
Wenn Sie sofort mit dem Tippen von Rust-Code beginnen, müssen Sie zwei komplexe Probleme gleichzeitig lösen:
...
Das Gehirn ist nicht dafür gemacht, beide Probleme simultan auf hohem Niveau zu lösen. Die Folge sind logische Fehler im Code, die schwer zu finden sind, da sie sich hinter Compiler-Warnungen oder Borrow-Checker-Konflikten verstecken. Sie verbringen Stunden damit, Code umzuschreiben, nur um festzustellen, dass Ihr grundlegender Ansatz falsch war.
...
Wenn Sie sich vor dem Programmieren 10 bis 15 Minuten Zeit nehmen, um den Ablauf visuell darzustellen und in Pseudocode aufzuschreiben, werden Sie:
* **Logikfehler frühzeitig erkennen:** Bevor Sie eine einzige Zeile Rust-Code schreiben, sehen Sie im Diagramm, ob Sie einen Fall (z. B. eine leere Eingabe) vergessen haben.
* **Denken auf einer höheren Ebene:** Im Pseudocode müssen Sie sich nicht um Semikolons, Typkonvertierungen oder Referenzen kümmern. Sie konzentrieren sich rein auf das "Was" und "Wie" des Algorithmus.
* **Weniger Frustration erleben:** Wenn die Logik einmal auf dem Papier steht, ist das Schreiben des Rust-Codes nur noch eine reine Übersetzungsarbeit. Der Compiler wird Ihr Freund, weil Sie sich voll auf die Syntax konzentrieren können.

### Warum Planung in Rust besonders wichtig ist
Rust ist eine sehr strikte Sprache. Der Borrow Checker wacht penibel über den Speicher. Wenn Sie in Rust ohne Plan drauflos programmieren, bauen Sie schnell Strukturen auf, die gegen die Aliasing-Regeln ("Ein Schreiber oder viele Leser") verstoßen. Ein guter Plan zeigt Ihnen vorab, welche Variablen verändert werden müssen (`mut`) und wo Daten ausgeliehen oder kopiert werden sollten.
...
Bevor Sie einen Algorithmus entwerfen, müssen Sie das Problem verstehen. Nutzen Sie das klassische **EVA-Prinzip**:
...
Bilder sagen mehr als tausend Worte. Ein Flussdiagramm (Flowchart) hilft Ihnen, Verzweigungen (`if/else`) und Schleifen (`loop`, `while`, `for`) visuell zu erfassen. In VS Code nutzen wir dafür **Mermaid**, ein Tool, mit dem man Diagramme direkt als Text definieren kann.
...
Bevor Sie den Pseudocode in Rust übersetzen, sollten Sie einen sogenannten "Trockenlauf" machen. Nehmen Sie sich ein Blatt Papier, schreiben Sie die Variablennamen auf und gehen Sie den Pseudocode Zeile für Zeile mit Beispielwerten im Kopf durch. Ändern Sie die Werte der Variablen auf Ihrem Blatt entsprechend. So merken Sie sofort, ob Ihre Schleife zum Beispiel ein Element zu früh stoppt (ein sogenannter "Off-by-one"-Fehler).
...
Der Pseudocode übersetzt das Diagramm in eine textuelle Struktur. Beachten Sie die Großbuchstaben bei den Kontrollstrukturen:
...
Verwenden Sie diese Spickzettel als Referenz für Ihre eigenen Projekte. Sie fassen die wichtigsten Elemente übersichtlich zusammen.
...
4. **Häufiger Anfängerfehler:**
   *Fehlerhafter Code:*
   ...
   ```
   *Korrektur:*
   Nutzen Sie immer doppelte Anführungszeichen für Sonderzeichen in Knotentexten: `Node{"Bedingung (x > 5)"}`.
...
4. **Häufiger Anfängerfehler:**
   *Fehlerhafter Pseudocode:*
   ...
   ```
   *Korrektur:*
   Nutzen Sie strukturierte Einrückungen und festgelegte Schlüsselwörter:
   ```text
   FALLS x > 10:
       MACH das_hier
   ```
...
Kopieren Sie diese Vorlage in jede Ihrer `planung.md`-Dateien, wenn Sie ein neues Projekt starten:
...
* **Das Problem:** Sie zeichnen einen Pfeil zurück zu einer Bedingung, verändern aber innerhalb der Schleife die Kontrollvariable nicht. Ihr Diagramm (und später Ihr Code) läuft ewig.
* **Vermeidung:** Stellen Sie bei jedem Schleifendurchlauf im Diagramm sicher, dass eine Aktion stattfindet, die die Bedingung beeinflusst (z. B. `Zähler = Zähler + 1`).

### Fehler 2: Fehlende Behandlung von Sonderfällen (Edge Cases)
* **Das Problem:** Sie planen nur für den Optimalfall (Happy Path). Was passiert, wenn der Notenrechner ein leeres Array übergeben bekommt? Im Rust-Code führt eine Division durch Null zum Absturz (Panic).
* **Vermeidung:** Fragen Sie sich bei jedem Schritt im Diagramm: *„Was ist, wenn der Wert null ist? Was ist, wenn das Array leer ist? Was ist bei negativen Werten?“* Bauen Sie entsprechende `if`-Abzweigungen ein.

### Fehler 3: Zu detaillierter Pseudocode (Rust-Syntax im Pseudocode)
* **Das Problem:** Sie schreiben bereits Typen wie `as f64`, Referenzen `&` oder Imports in Ihren Pseudocode. Das blockiert das kreative Denken über die Logik.
* **Vermeidung:** Halten Sie den Pseudocode einfach. Schreiben Sie `Länge des Arrays` statt `.len() as f64`. Die Details klären Sie erst beim Schreiben des Rust-Codes.

### Fehler 4: Die To-Do-Liste ist zu grob
* **Das Problem:** Ihre To-Do-Liste hat nur einen Punkt: *"1. Schreiben Sie das Programm"*. Das hilft Ihnen nicht bei der Strukturierung.
* **Vermeidung:** Zerlegen Sie die Aufgabe in kleine Portionen, die Sie in maximal 5 bis 10 Minuten erledigen können. Ein Punkt wie *"Schreiben Sie die Schleife für die Summe"* ist viel besser.

## 8.1 VS Code optimal für den Planungs-Workflow einrichten

Um den Planungs-Workflow so flüssig und angenehm wie möglich zu gestalten, sollten Sie Ihre VS Code-Umgebung mit den passenden Erweiterungen ausstatten. Dies ermöglicht es Ihnen, Diagramme und Markdown-Dateien direkt im Editor live zu visualisieren und zu bearbeiten.
...
1. **Markdown Preview Mermaid Support:**
   ...
   * *Nutzung:* Drücken Sie Strg + Shift + V in einer geöffneten Markdown-Datei, um die Live-Vorschau an der Seite anzuzeigen. Jede Änderung am Mermaid-Code wird sofort aktualisiert.
...
3. **Mermaid Chart:**
   * Die offizielle Erweiterung zur visuellen Erstellung und Bearbeitung von Mermaid-Diagrammen mit einem integrierten Editor, falls Sie die Syntax nicht auswendig lernen möchten.

### Hilfreiche Tastenkombinationen in VS Code für die Planung:
* **`Strg + Shift + V`:** Öffnet die Markdown-Vorschau im Vollbild.
* **`Strg + K, danach V`:** Öffnet die Vorschau in einem geteilten Fenster direkt neben Ihrem Code (Split-Screen-Modus). Dies ist das ideale Setup für die Implementierungsphase.
...
Während GitHub Copilot hervorragend als schneller Autocomplete-Assistent direkt in der Zeile arbeitet, glänzt das Antigravity CLI (`agy`) bei der projektweiten Planung und der Durchführung größerer Codeänderungen. Wenn Sie beide Tools intelligent kombinierst, entwickeln Sie ein tiefes Verständnis für Rust und vermeiden typische Fehler.

### Die Arbeitsteilung im Detail:
* **Der Navigator (Antigravity CLI):**
  - Nutzen Sie das CLI in der Planungsphase. Der Agent kann Ihnen helfen, logische Fehler in Ihrem Mermaid-Diagramm zu finden oder Ihren Entwurf in `planung.md` zu strukturieren.
  - Geben Sie dem CLI Befehle wie: *"Analysiere meinen Entwurf in planung.md und erstelle die grundlegenden Funktions-Signaturen in src/main.rs."*
* **Der Autopilot (GitHub Copilot):**
  - Nutzen Sie Copilot in der Tippphase innerhalb Ihrer Code-Dateien. Sobald Sie eine Funktion implementieren, schlägt Ihnen Copilot den passenden Code auf Zeilenebene vor.
  - *Achtung:* Verlassen Sie sich nicht blind auf Copilots Vorschläge. Wenn Copilot Code generiert, der nicht kompiliert, kopieren Sie die Fehlermeldung und fragen Sie das CLI nach einer strukturierten Erklärung des zugrunde liegenden Speicherproblems (z. B. Borrowing-Konflikte).
...
Nutzen Sie die KI, um Lücken in Ihrem Verständnis oder in den Anforderungen zu finden.
* **Tipp:** Beschreiben Sie der KI Ihr Vorhaben und fragen Sie nach Sonderfällen.
* **Beispiel-Prompt:**
...
Wenn Sie sich unsicher sind, wie der logische Fluss aussehen soll, können Sie die KI bitten, Ihnen einen Mermaid-Entwurf zu erstellen.
* **Tipp:** Wenn Sie bereits ein Diagramm haben, können Sie es auf Syntax- oder Logikfehler prüfen lassen.
* **Beispiel-Prompt:**
  > *"Erstellen Sie ein Mermaid-Flussdiagramm im 'graph TD'-Format für folgende Logik: [Logikbeschreibung, z. B. Summiere ein Array und ziehe bei über 50 Euro 10% Rabatt ab]. Geben Sie nur den Mermaid-Code aus."*

### Schritt 3: Pseudocode-Reviews (Logik-Prüfung)
Bevor Sie den ersten Rust-Code schreiben, sollten Sie Ihren Pseudocode von der KI überprüfen lassen. Da die KI hier keine Syntaxfehler korrigieren muss, kann sie sich ganz auf Logikfehler oder mögliche Endlosschleifen konzentrieren.
* **Beispiel-Prompt:**
  > *"Hier ist mein Pseudocode für [Aufgabe]: [Pseudocode einfügen]. Finden Sie darin logische Fehler, Endlosschleifen oder Fälle, in denen das Programm abstürzen könnte? Schlagen Sie Verbesserungen vor, ohne echten Code zu schreiben."*

### Schritt 4: To-Do-Checkliste erstellen
Sie können die KI bitten, Ihren Pseudocode in eine konkrete, kleinteilige Implementierungs-Checkliste für Rust zu übersetzen.
* **Beispiel-Prompt:**
  > *"Basierend auf diesem Pseudocode: [Pseudocode einfügen]. Erstellen Sie mir eine strukturierte To-Do-Liste mit kleinen Schritten für die Implementierung in Rust, beginnend bei 'cargo new'."*

### Schritt 5: Strukturierte Dokumentation der KI-Vorschläge
Es ist eine bewährte Praxis, die Ideen und Code-Vorschläge der KI nicht direkt blind in Ihr Projekt zu kopieren, sondern sie strukturiert in Ihrer `planung.md` zu sammeln.
* **Empfohlene Struktur in Ihrer `planung.md`:**
  ```markdown
  ...
  * **Entscheidung:** Ich wähle Idee A, um die imperativen Konzepte und den Dereferenzierungs-Operator `*` besser zu üben.
  ```
Das Sammeln und Abwägen dieser Vorschläge macht Sie zu einem mündigen Entwickler, der die Kontrolle über seinen Code behält, statt der KI blind zu vertrauen.

## 9. Übung: Der Warenkorb-Rabatt-Rechner

Jetzt sind Sie an der Reihe! Wenden Sie den gelernten Workflow an, um ein praktisches Problem zu lösen.
...
### Ihre Aufgabenschritte (Workflow)
1. Erstellen Sie einen neuen Ordner `warenkorb_rechner` und darin eine Datei `planung.md`.
2. Führen Sie die Anforderungen als EVA-Analyse in `planung.md` auf.
3. Erstellen Sie das Mermaid-Ablaufdiagramm in `planung.md`.
4. Schreiben Sie den Pseudocode auf.
5. Erstellen Sie die To-Do-Liste.
6. Erstellen Sie das Cargo-Projekt mit `cargo new warenkorb_rechner`.
7. Setzen Sie das Projekt in `src/main.rs` um. Testen Sie Ihr Programm mit verschiedenen Werten im Array (unter 50 € und über 50 €), um sicherzustellen, dass die Rabatt-Logik korrekt funktioniert!

## 9.1 Fünf weitere Planungs-Herausforderungen für Ihr Training

Um den Workflow in Fleisch und Blut übergehen zu lassen, finden Sie hier fünf Übungsszenarien mit unterschiedlichen Schwierigkeitsgraden. Wenden Sie bei jedem Szenario zuerst die 5 Schritte des Workflows an, bevor Sie programmieren.

### Challenge 1: Der Temperatur-Konverter (Einfach)
* **Ziel:** Ein Programm, das eine Liste von Celsius-Werten in Fahrenheit umrechnet.
* **Besonderheit:** Verwenden Sie ein festes Array von 5 Werten.
* **Fokus:** Einfache Schleife, mathematische Formel und Zuweisungen.

### Challenge 2: Der Palindrom-Tester (Mittel)
* **Ziel:** Prüfen Sie, ob ein eingegebenes Wort vorwärts und rückwärts gleich gelesen wird (z. B. "Lagerregal").
...
* **Ziel:** Simulieren Sie eine tickende Uhr im Terminal (Stunden, Minuten, Sekunden).
* **Besonderheit:** Nutzen Sie eine Endlosschleife und Thread-Sleep (`std::thread::sleep`) für den Sekundentakt.
* **Fokus:** Kontrollstrukturen und Zeitsteuerung.

### Challenge 4: Der Text-Zensor (Schwer)
* **Ziel:** Ersetzen Sie verbotene Wörter in einem Satz durch `***`.
...
* **Ziel:** Simulieren Sie Kontostandsabfragen, Einzahlungen und Auszahlungen über ein einfaches Menü.
* **Besonderheit:** Verwalten Sie den Kontostand über eine veränderliche Referenz in Funktionen.
* **Fokus:** Zustandssteuerung und Referenzen/Borrowing.

## 10. Musterlösung für den Warenkorb-Rabatt-Rechner

Vergleichen Sie Ihre Lösung erst mit dieser Musterlösung, nachdem Sie es selbst versucht haben. Das eigenständige Lösen und das Suchen nach Fehlern hat den größten Lerneffekt!
...
## 11. Zusammenfassung & Ihre Lernnotizen

Herzlichen Glückwunsch! Sie haben nun gelernt, wie wichtig es ist, komplexe Probleme strukturiert anzugehen und zu planen, bevor Sie Code schreiben. Der Planungs-Workflow wird Ihnen in Ihrer Karriere als Softwareentwickler extrem viel Zeit und Kopfzerbrechen ersparen.
...
2. **Klares Aufteilen:** Durch das EVA-Prinzip zerlegen Sie jedes Problem in seine drei Grundkomponenten.
3. **To-Do-Listen geben Struktur:** Kleine Teilaufgaben verhindern Überforderung und halten Sie fokussiert.

Nutzen Sie das untenstehende Feld in Ihrer eigenen Markdown-Datei, um Ihre Gedanken, Notizen und persönlichen Aha-Momente aus diesem Kapitel festzuhalten.
